...
class MarkovFormModel:
    ...
    def _build_transition_matrix(self, results: list[dict]) -> np.ndarray:
        ...
        # Results are ordered most recent first. To learn P(next | current),
        # we need transitions from current (more recent) to next (older).
        # e.g., if results = [R0, R1, R2] where R0 is most recent:
        #   R0 -> R1 is a transition (current=R0, next=R1)
        #   R1 -> R2 is a transition (current=R1, next=R2)
        for i in range(len(results) - 1):
            curr = self._result_to_state(results[i]["result"])       # more recent
            nxt = self._result_to_state(results[i + 1]["result"])    # older
            matrix[curr, nxt] += 1.0
